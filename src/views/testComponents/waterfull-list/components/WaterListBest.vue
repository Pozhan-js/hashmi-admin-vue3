<template>
  <div class="top fvc">
    <el-button type="primary" :disabled="page.loading" @click="getData(false)">
      {{ page.loading ? '加载中..' : '加载更多' }}
    </el-button>
    <el-button type="primary" :disabled="page.loading" @click="refresh">
      {{ page.loading ? '加载中..' : '重新获取' }}
    </el-button>
    <el-button type="primary" @click="page.move = !page.move">
      {{ page.move ? '关闭动画' : '开启动画' }}
    </el-button>
  ...
  >
    ...
    >
      ...
      <div class="content text-ellipsis text-ellipsis-2">{{ item.text }}</div>
    </div>
  </TransitionGroup>
</template>
...
//   -webkit-line-clamp: 2; /* N 是你想要显示的行数 */
// }
</style>
